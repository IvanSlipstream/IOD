...
function dateChooserSetDate(inputId, value) {
    ...
    if (input !== undefined && input.DateChooser !== undefined) {
		...
		parseInt(theForm.elements[prefix + 'sec'].options[
                    theForm.elements[prefix + 'sec'].selectedIndex].value)
		);
        }
...
DateChooser.prototype.setTime = function(hour, minute, second) {
...
//    result += "\r\n    <option value=\"0\">12</option>";
    ...
    result += "\r\n  <select name=\"" + this._prefix + "sec\">";
    for (var i = 0; i < 60; i ++) {
        result += "\r\n    <option value=\"" + i + "\""
            + ((this._date.getSeconds() == i) ? " selected=\"1\">" : ">")
            + String.leftPad(i, 2, '0') + "</option>";
    }
    result += "\r\n  </select>";
...
DateChooser.prototype.createCalendarHtml = function() {
    ...
    var firstDay = this._date.getFirstDayOfMonth()-1;
    var lastDay = this._date.getLastDayOfMonth()-1;
    if (firstDay != 0) {
